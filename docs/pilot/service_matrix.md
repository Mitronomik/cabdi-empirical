...
| Human-pilot backend | Participant API | `app/participant_api/main.py` | `uvicorn app.participant_api.main:app --reload --host 127.0.0.1 --port 8000` | `http://localhost:8000` | `PILOT_DB_PATH` optional; pilot configs/stimuli files | Runnable |
| Human-pilot backend | Researcher/Admin API | `app/researcher_api/main.py` | `uvicorn app.researcher_api.main:app --reload --host 127.0.0.1 --port 8001` | `http://localhost:8001` | `PILOT_DB_PATH` optional; stimulus upload content | Runnable |
