        ...
        output.append(
            {
                ...
                "verification_required": row.get("verification_required", 0),
                "verification_completed": row.get("verification_completed", 0),
                "reason_clicked": row.get("reason_clicked", 0),
                "evidence_opened": row.get("evidence_opened", 0),
                "switch_burden_proxy": row.get("switch_burden_proxy", 0),
                "reaction_time_ms": row.get("reaction_time_ms", 0),
                "log_rt": round(log_rt, 8) if log_rt is not None else None,
                "condition_n_trials": summary.get("n_trials"),
                "excluded_flagged": summary.get("excluded_flagged"),
