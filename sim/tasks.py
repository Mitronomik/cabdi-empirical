...
def _sample_catastrophic_weight(difficulty: float, rng: random.Random) -> float:
    base = 5.0 if difficulty > 0.72 else 1.0
    return base + (3.0 if rng.random() < 0.08 else 0.0)


def run_task_family(*, policy_name: str, policy_fn: PolicyFn, observation_mode: str, seed: int, episodes: int, horizon: int) -> list[StepRecord]:
    ...
    for ep in range(episodes):
        ...
        for t in range(horizon):
            ...
            catastrophic_weight = _sample_catastrophic_weight(difficulty, rng)
            obs = observe(state, difficulty, observation_mode, rng)
            action = policy_fn(obs)

            overload = max(state.load - 0.70, 0.0)
            ai_acc = max(0.05, min(0.95, 0.58 + 0.30 * action.help_level - 0.30 * difficulty - 0.26 * overload))
            human_acc = max(0.05, min(0.95, 0.67 - 0.25 * state.load + 0.20 * state.oversight - 0.28 * difficulty + 0.08 * action.verification_depth))
